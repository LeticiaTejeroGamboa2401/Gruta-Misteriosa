...
@onready var feedback_image: TextureRect = $Control/Acierto/Correcto

func _ready() -> void:
	var custom_cursor = load("res://images/Juegos/Derriba_al_Pato/mira_pato1.png")
	Input.set_custom_mouse_cursor(custom_cursor)
	
	MusicManager.play_music("res://audio/patos.ogg")

	# Guardar botones en arreglo
	for button in $Control/MenuPatitos.get_children():
...
func load_quiz() -> void:
	...
	for i in buttons.size():
		...
		# Escala opcional (ajusta si tus escenas son muy grandes)
		instance.scale = Vector2(1, 1)

		# Agregar la escena animada al botón
